<?php
namespace Opencart\Catalog\Model\Extension\CointopayFiat\Payment;

class CointopayFiat extends \Opencart\System\Engine\Model
{
	public function getMethods(array $address): array
	{
		...
		if (!$this->config->get('config_checkout_payment_address')) {
			$status = true;
		} elseif (!$this->config->get('payment_cointopay_fiat_geo_zone_id')) {
			$status = true;
		} else {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int) $this->config->get('payment_cointopay_fiat_geo_zone_id') . "' AND `country_id` = '" . (int) $address['country_id'] . "' AND (`zone_id` = '" . (int) $address['zone_id'] . "' OR `zone_id` = '0')");
		...
		if ($status) {
			$option_data['cointopay_fiat'] = [
				'code' => 'cointopay_fiat.cointopay_fiat',
				'name' => $this->config->get('payment_cointopay_fiat_display_name')
			];
